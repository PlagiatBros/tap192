...
tardiPiano::tardiPiano(int x, int y, int w, int h, const char* n) : Fl_Widget(x,y,w,h,n),lSelect(60),rSelect(60),mSelect(60)
{
    ...
    for(int i=0;i<10;++i) {
        float pos=0;
        //first part of the white keys
        for(int j=0;j<5;j+=2,++pos) {
            touches[12*i+j][0] =  (float)(7*i)*whiteW + pos*whiteW;
            touches[12*i+j][1] = 0;
        }

        //second part of the white keys
        for(int j=5;j<12;j+=2,++pos) {
            touches[12*i+j][0] = (float)(7*i)*whiteW + pos*whiteW;
            touches[12*i+j][1] = 0;
        }

        pos=1;
        //first part of the black keys
        for(int j=1;j<4;j+=2,++pos) {
            touches[12*i+j][0] = (float)(7*i)*whiteW +pos*whiteW - blackW/2.0;
            touches[12*i+j][1] = 1;

        }
        ++pos;
        //second part of the black keys
        for(int j=6;j<11;j+=2,++pos) {
            touches[12*i+j][0] = (float)(7*i)*whiteW + pos*whiteW - blackW/2.0;
            touches[12*i+j][1] = 1;
        }
    }
